    ...
    iterator updated(): Elf =
        for (elf, proposal) in proposals:
            if counted[proposal] == 1:
                ...
                yield elf
    elves = updated().toSeq().toHashSet()
    # ever seen a language where this assigns to the array one index at a time?
    # state = [state[1], state[2], state[3], state[0]]
    let temp = state[0]
    state = [state[1], state[2], state[3], temp]


let minx = toSeq(elves).map(e => e.x).foldl(min(a, b))
let miny = toSeq(elves).map(e => e.y).foldl(min(a, b))
let maxx = toSeq(elves).map(e => e.x).foldl(max(a, b))
let maxy = toSeq(elves).map(e => e.y).foldl(max(a, b))

echo((maxx - minx + 1) * (maxy - miny + 1) - elves.len())
