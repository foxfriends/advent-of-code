...
neighbours :: [[[[Bool]]]] -> (Int, Int, Int, Int) -> Int
neighbours space (x, y, z, w) = count [space !? ww >>= (!? zz) >>= (!? yy) >>= (!? xx) | xx <- [x-1..x+1],
                                                                                         yy <- [y-1..y+1],
                                                                                         zz <- [z-1..z+1],
                                                                                         ww <- [w-1..w+1],
                                                                                         xx /= x || yy /= y || zz /= z || ww /= w]
...
run :: [[[[Bool]]]] -> [[[[Bool]]]]
run hyperspace = [[[[next (x, y, z, w) value | (x, value) <- [0..] `zip` row]
                                             | (y, row) <- [0..] `zip` plane]
                                             | (z, plane) <- [0..] `zip` space]
                                             | (w, space) <- [0..] `zip` hyperspace]
    where
        next :: (Int, Int, Int, Int) -> Bool -> Bool
        next = update . neighbours hyperspace
...
main = do
    ...
    print . length . filter id . join . join . join $ cycles 6 [[contents]]
